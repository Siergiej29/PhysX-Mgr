...
static PxPhysics*				g_PhysicsSDK = NULL;			//Instance of PhysX SDK
...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    g_hWnd = CreateWindow( L"TutorialWindowClass", L"Real-Time Computer Graphics - IMII UO 2016", WS_OVERLAPPEDWINDOW,
...
//HeightField
	g_Terrain.HeightMapLoad("Teren.bmp", 5.0, 5.0, 12.0);
...
void InitPhysX() 
{
	...
	PxTransform		spherePos1(PxVec3(0.0f, 22.0f, 0.0f));	
	...
	PxTransform		spherePos2(PxVec3(3.0f, 22.0f, 0.0f));	
	...
	PxTransform		spherePos3(PxVec3(-3.0f, 22.0f, 0.0f));	
	...
	PxShape* aHeightFieldShape = g_PxHeightfield->createShape(hfGeom, *material);
					g_PxScene->addActor(*g_PxHeightfield);
}
...
//---RENDER SCENE---
 ...
 //   g_pd3dDevice->ClearRenderTargetView( g_pRenderTargetView, ClearColor ); // Nie czyœcimy bufora koloru, bo i tak zamalujemy ca³y ekran niebem
    ...
    g_pd3dDevice->IASetIndexBuffer( g_pHeightFieldIndexBuffer, DXGI_FORMAT_R32_UINT, 0 );
    g_pd3dDevice->IASetPrimitiveTopology( D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
    g_pNormalMap1Technique->GetPassByIndex( 0 )->Apply( 0 );
	g_pd3dDevice->DrawIndexed( 3 * g_Terrain.NumFaces, 0, 0 );
