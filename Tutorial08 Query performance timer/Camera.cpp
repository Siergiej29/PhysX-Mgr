...
********************************************************************/
void Camera::setPerspectiveProjectionLH(float iFOV, float iaspectRatio, float izNear, float izFar)
{
	//convert FOV from degrees to radians
	FOV = iFOV * (float) DEG_TO_RAD;
...
{	
	...
	D3DXVECTOR3 view2 = eye + view;
	
	//update view matrix
	D3DXMatrixLookAtLH( &viewMatrix, &eye, &view2, &up );
}
...
void Camera::update(double dt)
{
	...
	eye += (float)dt * (movementToggles[0] - movementToggles[1]) * movementSpeed * view +
		(float)dt * (movementToggles[2] - movementToggles[3]) * movementSpeed * right;
